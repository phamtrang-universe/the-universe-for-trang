document.addEventListener('DOMContentLoaded', () => {
    ...
    let drawState = {
        ...
        completed: false,    
        progress: 0,         
        speed: 0.02 // Tốc độ tự vẽ (càng nhỏ càng chậm)
    };
    ...
    // Tọa độ Bọ Cạp đã tinh chỉnh để nằm gọn trong vùng 70% bên dưới
    const SCORPIO_DATA = [
    ...
    function initConstellation() {
        // Căn giữa theo chiều ngang và dọc của canvas dưới
    ...
    function renderIntro() {
        ...
        if (drawState.started && !drawState.completed) {
            ...
            if (drawState.progress >= constellation.length - 1) {
                ...
                setTimeout(transitionToMainJourney, 1500); // Vẽ xong đợi 1.5s rồi chuyển
            }
        }
        ...
        constellation.forEach((p, idx) => {
            if (idx === 0 || idx <= Math.floor(drawState.progress)) {
                ...
                // Hiệu ứng Pulse cho điểm đầu khi chờ
