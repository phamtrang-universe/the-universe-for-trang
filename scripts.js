document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkPassword() {
        if (passInput.value.trim() === ACCESS_CODE) unlockUniverse();
        else { errorMsg.style.opacity = '1'; passInput.value = ""; setTimeout(() => errorMsg.style.opacity = '0', 2000); }
    }
    ...
    function resize() {
        ...
        if (container) {
            ...
            if(isIntroActive) { initConstellation(); if(stars.length===0) initBackgroundStars(); }
        }
    }
    ...
    function initBackgroundStars() {
        stars = [];
        for(let i=0; i<80; i++) stars.push({x:Math.random()*w, y:Math.random()*h, size:Math.random()*1.5, opacity:Math.random(), speed:Math.random()*0.05+0.01});
    }

    const SCORPIO_DATA = [{x:0.7,y:0.1}, {x:0.6,y:0.2}, {x:0.5,y:0.3}, {x:0.45,y:0.45}, {x:0.5,y:0.6}, {x:0.6,y:0.75}, {x:0.75,y:0.8}, {x:0.85,y:0.7}, {x:0.9,y:0.65}];
    function initConstellation() {
        constellation = SCORPIO_DATA.map(p => ({
            x: (w*0.5) + (p.x-0.65)*(w*0.8), y: (h*0.1) + (p.y)*(h*0.75), r: (p===SCORPIO_DATA[3])?6:4, isAntares: (p===SCORPIO_DATA[3])
        }));
    }
    ...
    function renderIntro() {
        ...
        stars.forEach(s => { s.y-=s.speed; if(s.y<0)s.y=h; introCtx.globalAlpha=s.opacity; introCtx.beginPath(); introCtx.arc(s.x,s.y,s.size,0,Math.PI*2); introCtx.fill(); });

        if (drawState.started && !drawState.completed) {
            drawState.progress += drawState.speed;
            if (drawState.progress >= constellation.length - 1) {
                drawState.progress = constellation.length - 1; drawState.completed = true;
                setTimeout(transitionToMainJourney, 1000);
            }
        }
        if (drawState.progress > 0) {
            ...
            const lastIndex = Math.floor(drawState.progress); const t = drawState.progress - lastIndex;
    ...
    function transitionToMainJourney() {
        openingScreen.style.transition = "opacity 2s ease"; openingScreen.style.opacity = 0;
        setTimeout(() => {
            openingScreen.classList.add('hidden'); isIntroActive = false;
            mainJourney.classList.remove('hidden'); mainJourney.classList.add('visible');
            initMainJourney();
        }, 2000);
    }
    ...
    class GalaxyStar {
        ...
        draw() {
            ...
            const y = centerY + Math.sin(this.angle + this.spiralAngle) * r; 
